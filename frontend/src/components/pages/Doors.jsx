...
function Doors({ showToast }) {
  ...
  const filterBySearch = (items) => {
    ...
    return items.filter((item) => {
      ...
      if (item.ip_address && item.ip_address.toLowerCase().includes(query)) return true;
      // Search in ID
      if (item.id && String(item.id).toLowerCase().includes(query)) return true;
      // Search in building name
      const building = groups.find((g) => g.id === item.building_id);
      if (building && building.name && building.name.toLowerCase().includes(query)) return true;
      return false;
    });
  };
  ...
  const getFilteredDoors = () => {
    return filterBySearch(doors);
  };

  // Get grouped doors with search
  const getGroupedDoors = () => {
    ...
    return groups
      .map((group) => ({
        group,
        doors: filteredDoors.filter((door) => door.building_id === group.id),
      }))
      .filter((item) => item.doors.length > 0);
  };

  // Get count for display
  const getFilteredCount = () => getFilteredDoors().length;
  ...
  return (
    <div className="pro-page">
      ...
      <div className="door-search-bar">
        <div className="search-input-wrapper">
          <i className="fas fa-search"></i>
          <input
            type="text"
            className="form-control search-input"
            placeholder="Search doors by name, location, building, IP, or ID..."
            value={searchQuery}
        ...
        {searchQuery && (
          <div className="search-results-info">
            ...
            Showing {getFilteredCount()} of {doors.length} doors
          </div>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="pro-empty-state">
          <div>
            <div className="pro-empty-icon mx-auto">
              <i className={searchQuery ? "fas fa-search" : "fas fa-door-closed"}></i>
            </div>
            <h5>{searchQuery ? "No matching doors found" : "No doors configured"}</h5>
            <p>
              {searchQuery
                ? "Try adjusting your search terms."
                : "Add a door to connect access permissions with a building."}
            </p>
          </div>
        </div>
      ) : (
